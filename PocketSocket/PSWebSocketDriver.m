...
+ (BOOL)isWebSocketRequest:(NSURLRequest *)request {
  ...
  NSOrderedSet *version = PSHTTPHeaderFieldValues(
      [headers[@"Sec-WebSocket-Version"] lowercaseString]);
  NSOrderedSet *upgrade =
      PSHTTPHeaderFieldValues([headers[@"Upgrade"] lowercaseString]);
  NSOrderedSet *connection =
      PSHTTPHeaderFieldValues([headers[@"Connection"] lowercaseString]);

  if (headers[@"Sec-WebSocket-Key"] && [version containsObject:@"13"] &&
...
- (instancetype)initWithMode:(PSWebSocketMode)mode
                     request:(NSURLRequest *)request {
  NSParameterAssert(request);
  if ((self = [super init])) {
    _mode = mode;
    _state = (_mode == PSWebSocketModeClient)
                 ? PSWebSocketDriverStateHandshakeRequest
                 : PSWebSocketDriverStateHandshakeResponse;
...
- (NSUInteger)execute:(void *)bytes maxLength:(NSUInteger)maxLength {
  ...
  while (totalBytesRead < maxLength) {
    bytesRead = [self readBytes:bytes
                      maxLength:maxLength - totalBytesRead
                          error:&error];
    if (bytesRead < 0) {
      if (error) {
...
- (void)sendCloseCode:(NSInteger)code reason:(NSString *)reason {
  NSUInteger reasonMaxLength =
      [reason maximumLengthOfBytesUsingEncoding:NSUTF8StringEncoding];
  NSMutableData *data =
      [NSMutableData dataWithLength:sizeof(uint16_t) + reasonMaxLength];
  ...
  if (reason) {
    ...
    __unused BOOL success =
        [reason getBytes:dataBytes + sizeof(uint16_t)
    ...
    NSAssert(remainingRange.length == 0,
             @"Failed to write reason when sending close frame");
...
- (void)writeHandshakeRequest {
  NSAssert(_mode == PSWebSocketModeClient,
           @"Cannot send handshake requests in non client mode");
  ...
  int result = SecRandomCopyBytes(kSecRandomDefault, secKeyData.length,
                                    secKeyData.mutableBytes);
  if (result != 0) {
  ...
  NSString *host =
      (URL.port) ? [NSString stringWithFormat:@"%@:%@", URL.host, URL.port]
                 : URL.host;
  NSString *origin =
      [NSString stringWithFormat:@"http%@://%@", (secure) ? @"s" : @"", host];
  ...
  CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Sec-WebSocket-Version"),
                                   CFSTR("13"));
  CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Sec-WebSocket-Key"),
                                   (__bridge CFStringRef)_handshakeSecKey);
  CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Origin"),
                                   (__bridge CFStringRef)origin);

  [_request.allHTTPHeaderFields
      enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        CFHTTPMessageSetHeaderFieldValue(
            msg, (__bridge CFStringRef)[NSString stringWithFormat:@"%@", key],
            (__bridge CFStringRef)[NSString stringWithFormat:@"%@", obj]);
      }];

  // extensions
  NSMutableArray *extensionComponents = [NSMutableArray array];
  [extensionComponents
      addObjectsFromArray:[self pmdExtensionsHeaderComponents]];
  if (extensionComponents.count > 0) {
    NSString *value = [extensionComponents componentsJoinedByString:@"; "];
    CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Sec-WebSocket-Extensions"),
                                     (__bridge CFStringRef)value);
  }

  // serialize
  NSData *handshakeData =
      CFBridgingRelease(CFHTTPMessageCopySerializedMessage(msg));
...
- (void)writeHandshakeResponse {
  NSAssert(_state == PSWebSocketDriverStateHandshakeResponse,
           @"Cannot start a driver more than once");
  ...
  // validate extensions
  NSOrderedSet *extensionComponents = PSHTTPHeaderFieldValues(
      [headers[@"Sec-WebSocket-Extensions"] lowercaseString]);
  if (![self pmdConfigureWithExtensionsHeaderComponents:extensionComponents]) {
    [self failWithErrorCode:PSWebSocketErrorCodeHandshakeFailed
                     reason:@"invalid permessage-deflate extension parameters"];
    return;
  }
  ...
  CFHTTPMessageRef msg = CFHTTPMessageCreateResponse(
      kCFAllocatorDefault, 101, CFSTR("Switching Protocols"),
      kCFHTTPVersion1_1);
  CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Connection"), CFSTR("Upgrade"));
  CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Upgrade"), CFSTR("websocket"));
  CFHTTPMessageSetHeaderFieldValue(
      msg, CFSTR("Sec-WebSocket-Accept"),
      (__bridge CFStringRef)[self acceptHeaderForKey:_handshakeSecKey]);
  if (_protocol) {
    CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Sec-WebSocket-Protocol"),
                                     (__bridge CFStringRef)_protocol);
  }

  NSMutableArray *negotiatedExtensionComponents = [NSMutableArray array];
  [negotiatedExtensionComponents
      addObjectsFromArray:[self pmdExtensionsHeaderComponents]];
  if (negotiatedExtensionComponents.count > 0) {
    NSString *value =
        [negotiatedExtensionComponents componentsJoinedByString:@"; "];
    CFHTTPMessageSetHeaderFieldValue(msg, CFSTR("Sec-WebSocket-Extensions"),
                                     (__bridge CFStringRef)value);
  }
...
- (void)writeMessageWithOpCode:(PSWebSocketOpCode)opcode data:(NSData *)data {
  ...
  // deflate payload
  if (_pmdEnabled && !PSWebSocketOpCodeIsControl(opcode) &&
      [payload length] > 0) {
    ...
    // append bytes
    if (![_deflater appendBytes:[payload bytes]
  ...
  if (_mode == PSWebSocketModeClient) {
    headerBytes =
        header.mutableBytes; // because -appendBytes may have realloced header
...
- (NSInteger)readBytes:(void *)bytes
  ...
  case PSWebSocketDriverStateHandshakeResponse: {
    NSAssert(maxLength > 0, @"Must have 1 or more bytes");
    NSAssert(_state == PSWebSocketDriverStateHandshakeResponse,
             @"Invalid state for reading handshake response");

    void *boundary = memmem(bytes, maxLength, "\r\n\r\n", 4);
    if (boundary == NULL) {
      // do not allow too much data for headers
      if (maxLength >= 16384) {
        PSWebSocketSetOutError(
            outError, PSWebSocketErrorCodeHandshakeFailed,
            @"HTTP headers did not finish after reading 16384 bytes");
        return -1;
      }
      return 0;
    }
    ...
    if (!CFHTTPMessageAppendBytes(msg, (const UInt8 *)bytes,
                                  preBoundaryLength)) {
    ...
    if (!CFHTTPMessageIsHeaderComplete(msg)) {
      PSWebSocketSetOutError(outError, PSWebSocketErrorCodeHandshakeFailed,
                             @"HTTP headers found CRLFCRLF but not complete");
      CFRelease(msg);
      return -1;
    }
    ...
    NSDictionary *headers =
        [CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(msg)) copy];
    CFAutorelease(msg);
    ...
    if (statusCode != 101) {
      if (outError) {
        NSString *message =
            CFBridgingRelease(CFHTTPMessageCopyResponseStatusLine(msg));
        if (!message)
        ...
        NSString *desc =
            [@"Handshake failed: " stringByAppendingString:message];
    ...
    if (![headers[@"Sec-WebSocket-Accept"]
            isEqualToString:[self acceptHeaderForKey:_handshakeSecKey]]) {
      PSWebSocketSetOutError(outError, PSWebSocketErrorCodeHandshakeFailed,
    ...
    NSString *protocolRequest =
        _request.allHTTPHeaderFields[@"Sec-WebSocket-Protocol"];
    if (protocolRequest) {
      NSArray *protocolComponents =
          [protocolRequest componentsSeparatedByString:@" "];
    ...
    // per-message deflate
    if (![self
            pmdConfigureWithExtensionsHeaderComponents:extensionComponents]) {
      PSWebSocketSetOutError(
          outError, PSWebSocketErrorCodeHandshakeFailed,
          @"permessage-deflate could not negotiate parameters");
      return -1;
    }
  ...
  case PSWebSocketDriverStateFrameHeader: {
    ...
    if (!PSWebSocketOpCodeIsValid(opcode)) {
      PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                             @"Invalid opcode");
      return -1;
    }
    ...
    if (control) {
      ...
      if (rsv1 || rsv2 || rsv3) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                               @"Control frames must not use reserved bits");
        return -1;
      }
      // control frames must not have payload >= 126
      if (payloadLength >= 126) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                               @"Control frames payload must < 126");
        return -1;
      }
    }
    // validate data frame
      ...
      if (opcode == PSWebSocketOpCodeContinuation && _frames.count == 0) {
        PSWebSocketSetOutError(
            outError, PSWebSocketStatusCodeProtocolError,
            @"Data continuation frames must follow an initial data frame");
        return -1;
      }
      ...
      if (opcode != PSWebSocketOpCodeContinuation && _frames.count > 0) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                               @"Data frames must not follow an initial data "
                               @"frame unless continuations");
        return -1;
      }
      // rsv1 only set if permessage-deflate is enabled
      if (rsv1 && !_pmdEnabled) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                               @"Data frames must only use rsv1 bit if "
                               @"permessage-deflate extension is on");
        return -1;
      }
      // rsv2 and rsv3 must never be set
      if (rsv2 || rsv3) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                               @"Data frames must never use rsv2 or rsv3 bits");
        return -1;
      }
    }
    ...
    if (masked && _mode == PSWebSocketModeClient) {
      PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                             @"Frames must never be masked from server");
      return -1;
    }
    // ensure masked if server mode
    if (!masked && _mode == PSWebSocketModeServer) {
      PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                             @"Frames must always be masked from client");
      return -1;
    }
  ...
  case PSWebSocketDriverStateFrameHeaderExtra: {
    ...
    if (frame->masked) {
      memcpy(frame->maskKey,
             (uint8_t *)bytes + (frame->headerExtraLength - sizeof(uint32_t)),
             sizeof(uint32_t));
      frame->maskOffset = 0;
    }
  ...
  case PSWebSocketDriverStateFramePayload: {
    ...
    if (_mode == PSWebSocketModeServer) {
      ...
      for (NSInteger i = 0; i < consumeLength; ++i) {
        unmaskedBytes[i] =
            unmaskedBytes[i] ^ maskKey[frame->maskOffset++ % sizeof(uint32_t)];
      }
    }
    ...
    if (frame->opcode == PSWebSocketOpCodeText) {
      ...
      for (NSUInteger i = 0; i < frame->buffer.length - offset; ++i) {
        ...
        PSWebSocketUTF8DecoderDecode(&_utf8DecoderState, &_utf8DecoderCodePoint,
                                     *(bytes + i));

        // read bad code point
        if (_utf8DecoderState == PSWebSocketUTF8DecoderReject) {
          PSWebSocketSetOutError(outError, PSWebSocketStatusCodeInvalidUTF8,
                                 @"Invalid UTF-8");
          return -1;
        }
      }
      ...
      if (_utf8DecoderState > 1 && frame->fin &&
          frame->payloadRemainingLength - consumeLength == 0) {
        PSWebSocketSetOutError(outError, PSWebSocketStatusCodeInvalidUTF8,
                               @"Invalid UTF-8");
        return -1;
      }
...
- (BOOL)processFramesAndDelegate:(NSError *__autoreleasing *)outError {
  ...
  // close off pmd for zero-length frames that have a buffer otherwise they are
  // orphaned
  ...
  case PSWebSocketOpCodeText: {
    NSString *utf8 = [[NSString alloc] initWithData:frame->buffer
                                           encoding:NSUTF8StringEncoding];
    if (!utf8) {
      PSWebSocketSetOutError(outError, PSWebSocketStatusCodeInvalidUTF8,
                             @"Invalid UTF-8");
    ...
    if (frame->buffer.length >= 2) {
      ...
      if (frame->buffer.length > 2) {
        reason = [[NSString alloc]
            initWithBytes:frame->buffer.mutableBytes + sizeof(uint16_t)
                   length:frame->buffer.length - sizeof(uint16_t)
                 encoding:NSUTF8StringEncoding];
        if (!reason) {
          PSWebSocketSetOutError(outError, PSWebSocketStatusCodeProtocolError,
                                 @"Invalid close reason; must be UTF-8");
          return NO;
        }
      }
      ...
      [_delegate driver:self
          didCloseWithCode:PSWebSocketStatusCodeNoStatusReceived
                    reason:nil];
    }
    break;
...
- (NSArray *)pmdExtensionsHeaderComponents {
  if (_pmdEnabled) {
    NSMutableArray *components =
        [NSMutableArray arrayWithObject:@"permessage-deflate"];
    ...
    // server mode
      ...
      [components
          addObject:[NSString stringWithFormat:@"client_max_window_bits=%@",
                                               @(-_pmdClientWindowBits)]];

      // set the window bits the server will use
      [components
          addObject:[NSString stringWithFormat:@"server_max_window_bits=%@",
                                               @(-_pmdServerWindowBits)]];
    }
    return components;
  }
  return @[];
...
- (BOOL)pmdConfigureWithExtensionsHeaderComponents:(NSOrderedSet *)components {
  ...
  for (NSString *component in components) {
    ...
    if ([subcomponents[0] isEqualToString:@"permessage-deflate"]) {
      _pmdEnabled = YES;
    } else if ([subcomponents[0] isEqualToString:@"client_max_window_bits"] &&
               subcomponents.count > 1) {
      _pmdClientWindowBits = -[subcomponents[1] integerValue];
    } else if ([subcomponents[0] isEqualToString:@"server_max_window_bits"] &&
               subcomponents.count > 1) {
      _pmdServerWindowBits = -[subcomponents[1] integerValue];
    } else if ([subcomponents[0]
                   isEqualToString:@"client_no_context_takeover"] &&
               _mode == PSWebSocketModeClient) {
      _pmdClientNoContextTakeover = YES;
    } else if ([subcomponents[0]
                   isEqualToString:@"server_no_context_takeover"] &&
               _mode == PSWebSocketModeClient) {
  ...
  if (_pmdEnabled) {
    if (_mode == PSWebSocketModeClient) {
      _inflater =
          [[PSWebSocketInflater alloc] initWithWindowBits:_pmdServerWindowBits];
      _deflater =
          [[PSWebSocketDeflater alloc] initWithWindowBits:_pmdClientWindowBits
                                              memoryLevel:8];
    } else {
      _inflater =
          [[PSWebSocketInflater alloc] initWithWindowBits:_pmdClientWindowBits];
      _deflater =
          [[PSWebSocketDeflater alloc] initWithWindowBits:_pmdServerWindowBits
                                              memoryLevel:8];
    }
  }
...
// if we're targeting deployment before OS X 10.9 or IOS 7 the public methods
// for base 64 encoding didn't exist
// however, a more basic private API ( which is now also public but deprecated )
// did exist so we'll use it instead
